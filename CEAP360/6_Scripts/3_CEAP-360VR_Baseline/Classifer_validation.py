...
# 1) Load data and prepare training and test data
# 2) Construct subjective dependent (SD) and subjective-independent (SI) models
...
def load_data(seg, subject_id, feature, label_id, SD):
    if feature == 0:
        path = '/Users/emilydoherty/Desktop/IRES/Emotion_Datasets/CEAP-360VR-Dataset-master/CEAP-360VR/sub_split/'
        range_index = [0, 12]
    else:
        path = './sub_split_feature_' + str(seg) + '/'
        range_index = [0, 55]  # feature num = 49
    ...
    if feature == 0:
        train_x = train_x.reshape([-1, 50 * seg, range_index[1] - range_index[0]])  # 50Hz*2s
        test_x = test_x.reshape([-1, 50 * seg, range_index[1] - range_index[0]])
        train_y = train_y.reshape([-1, 50 * seg])
        train_y = np.round(np.mean(train_y, axis=1)).reshape([-1, 1])
        test_y = test_y.reshape([-1, 50 * seg])
    ...
    for j in range(10):  # number of folds
        ...
        for l in range(int(max(y) + 1)):
            ...
            train_y = np.concatenate((train_y, np.delete(yy[l].reshape([-1, 1]),
                                     ...
                                     axis=0)
        acc_train, acc_test, f1_test = Classifer_models.to_result(seg, model_type, train_x, train_y, test_x, test_y)
